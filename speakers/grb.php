...
$spkfac 		= (isset($_POST['spkfac']))	? trim($_POST['spkfac']) 	: '';
$spktwi 		= (isset($_POST['spktwi']))	? trim($_POST['spktwi']) 	: '';
$spkins 		= (isset($_POST['spkins']))	? trim($_POST['spkins']) 	: '';
...
$spkfac      	= VarNullBD($spkfac, 'S');
$spktwi      	= VarNullBD($spktwi, 'S');
$spkins      	= VarNullBD($spkins, 'S');
...
if ($spkreg == 0) {
	...
	$query = " 	INSERT INTO SPK_MAEST (SPKREG,SPKNOMBRE,SPKDESCRI,SPKPOS,ESTCODIGO,SPKEMPRES,SPKCARGO,SPKLINKED, SPKFAC, SPKTWI, SPKINS)
					VALUES ($spkreg,$spktitulo,$spkdescri,$spkpos,$estcodigo,$spkempres,$spkcargo, $spklinked, $spkfac, $spktwi, $spkins) ";
} else {
	$query = " 	UPDATE SPK_MAEST SET 
					SPKNOMBRE=$spktitulo,SPKDESCRI=$spkdescri, SPKPOS=$spkpos, ESTCODIGO=$estcodigo,
					SPKEMPRES=$spkempres,SPKCARGO=$spkcargo, SPKLINKED=$spklinked, SPKFAC=$spkfac,SPKTWI=$spktwi, SPKINS=$spkins
					WHERE SPKREG=$spkreg ";
}
//logerror($query);
//die();
