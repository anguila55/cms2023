...
<?
	...
	$encobliga 		= (isset($_POST['encobliga']))? trim($_POST['encobliga']) : 0;
	...
	$encobliga	= VarNullBD($encobliga		, 'N');
	...
	if($encreg==0){
		...
		$query = " 	INSERT INTO ENC_CABE(ENCREG,ENCDESCRI,ENCFCHREG,ESTCODIGO,ENCPUBLIC,OBLIGA)
					VALUES($encreg,$encdescri,CURRENT_TIMESTAMP,$estcodigo,$encpublic,$encobliga) ";
	}else{
		$query = " 	UPDATE ENC_CABE SET 
					ENCDESCRI=$encdescri,ENCFCHREG=CURRENT_TIMESTAMP,ESTCODIGO=$estcodigo,
					ENCPUBLIC=$encpublic, OBLIGA=$encobliga
					WHERE ENCREG=$encreg ";
	}
