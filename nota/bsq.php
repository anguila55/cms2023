...
<?
	//--------------------------------------------------------------------------------------------------------------
	//Valido si el perfil existe
	if (!defined('GLBAPPPORT')) define('GLBAPPPORT', $_SERVER['SERVER_PORT']);
	
	$notahome1 = (isset($_GET['ID']))? trim($_GET['ID']):''; //Nota desde el home acceso directo
	if($notahome1!=''){
		$vaux = explode('||',$notahome1);
		if(count($vaux)>1){
			$notahome = $vaux[0];
		}
	}
	
	if (!isset($_SESSION[GLBAPPPORT.'PERCODIGO']) || trim($_SESSION[GLBAPPPORT.'PERCODIGO'])=='' ){
		//Si el perfil no existe, fuerzo un login, para luego redireccionar
		echo "<script> window.top.location='../index?ID=$notahome1'; </script>";
		exit;
	}
	...
	if($notahome!=''){
		$tmpl->setVariable('scriptmetricas'	, "showMetricasNota($notahome);"	);
		$tmpl->setVariable('scriptbrwprofile'	, "showBrowserNota($notahome);"	);
		$tmpl->setVariable('notareg'	, $notahome	);
	}
