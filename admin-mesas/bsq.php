...
<?
	...
	$query = "	SELECT PC.PERCLASE,PC.PERCLADES,PC.PERTIPO,PT.PERTIPDESESP
					FROM PER_CLASE PC
					LEFT OUTER JOIN PER_TIPO PT ON PT.PERTIPO=PC.PERTIPO 
					WHERE PT.ESTCODIGO<>3 AND PC.ESTCODIGO<>3
					ORDER BY PC.PERCLADES ";
		$Table = sql_query($query,$conn);
		...
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			$pertipodes	= trim($row['PERTIPDESESP']);
			$pertipocod	= trim($row['PERTIPO']);

			$tmpl->setCurrentBlock('perfiles');
			$tmpl->setVariable('perclacod'	, $perclacod 	);
			$tmpl->setVariable('perclades'	, $pertipodes.' ( '.$perclades.' )'	);
			$tmpl->parse('perfiles');
