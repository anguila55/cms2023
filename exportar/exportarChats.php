...
<?
	...
	elseif($desde && !$hasta){
		$and = "AND CHAFCHREG > '$desde'";
	}elseif(!$desde && $hasta){
		$and = "AND CHAFCHREG < '$hasta'";
	}else{
		$and = "AND CHAFCHREG BETWEEN '$desde' AND '$hasta'";
	}
    $conn= sql_conectar();//Apertura de Conexion
        ...
        $query="SELECT DISTINCT PS.PERCOMPAN AS SOLEMPRESA,PS.PERNOMBRE AS SOLNOMBRE,PS.PERAPELLI AS SOLAPELLI,PS.PERCORREO AS SOLCORREO,
				...
				WHERE PS.PERCODIGO IS NOT NULL AND PD.PERCODIGO IS NOT NULL $and
