...
var cantidadProductosMaximos = $("#catprod").val();
...
$("#confirmar-producto").click((e) => {
  ...
  formData.append("expreg", $("#sponsor_id").val() );
  formData.append("catprod", cantidadProductosMaximos);
...
function agregarNuevoProducto() {
  $("#lista-productos").append(
    `<h6 class="card-title mb-2 mt-3">Producto</h6>
        ...
        <input placeholder="Descripcion" type="text" id="proddes"
            class="form-control mb-3 d-none" value="">
        <div class="col-md-12">
...
function productoConfirmado(valNombre,valDescrip, valImagen) {
  $("#nuevos-productos").append(`
    <h4 class="card-title text-center mt-3 mb-4">${valNombre}</h4>
    <p class="card-title text-center mt-3 mb-4" style="font-size: medium;">${valDescrip}</p>
...
function guardarProducto(formData) {
  $.ajax({
    ...
    if (data.errcod == 0) {
      ...
      confirmProduct = 1;
      productoConfirmado(formData.get("prodnombre"),formData.get("proddes"), data.imgname);
      acumuladorProductos++;
    } else {
