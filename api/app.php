...
<?
	...
	define('mailNameApp', 'CONGRESO AAPRESID');
	define('SendMailNombre', 'CONGRESO AAPRESID');
	define('SendMailUsuario', 'inscripciones@aapresid.org.ar');
	define('SendMailPass', 'congreso2020');
	
	define('SendMailReporteReuniones', 'fernando.pereyra@onlife.com.ar');

	define('LinkPlayStore', 'https://play.google.com/store/apps/details?id=com.nextar.aapresid');
	define('LinkAppStore', 'https://apps.apple.com/us/app/id1527561064'); //https://apps.apple.com/ve/app/cms-365/id1465806972
		...
		if($tipologin==1 || $tipologin==0){  //Login Registrados
			if($perusuacc!=''){
				$query = "	SELECT P.PERCODIGO,P.PERNOMBRE,P.PERAPELLI,P.PERCOMPAN,P.PERAVATAR,
									...
									P.PERIDIOMA,IM.IDIDESCRI,P.PERCIUDAD,P.PERDIRECC,P.PERESTADO
							FROM PER_MAEST P
							...
							WHERE (P.ESTCODIGO=1 AND P.PERUSUACC='$perusuacc' AND P.PERPASACC='$perpasacc')";
				...
				if($Table->Rows_Count>0){
					...
					$reg['Direccion'] 				= trim($row['PERDIRECC']);
...
$app->post('/directorio', function()  use ($app){
	...
	try{
		...
		$query = "	SELECT FIRST $cantPagina SKIP $pagina P.PERCODIGO,P.PERNOMBRE,P.PERAPELLI,P.PERCOMPAN,P.PERURLWEB,P.PEREMPDES,P.PERCARGO,
							P.PERAVATAR,P.PERINS,P.PERFAC,P.PERTWI,P.PERCIUDAD,P.PERESTADO,
							(SELECT FIRST 1 E.EXPSTAND
                            FROM EXP_MAEST E
                            WHERE E.PERCODIGO=P.PERCODIGO) AS MESNUMERO,
							P.PERTIPO,T.PERTIPDESESP,
							P.PERCLASE,C.PERCLADES,
...
$app->post('/sponsors', function()  use ($app){
	...
	try{
		...
		$query = "	SELECT FIRST $cantPagina SKIP $pagina E.EXPREG,E.EXPNOMBRE,E.EXPWEB,E.EXPMAIL,E.EXPSTAND,E.EXPRUBROS,
							...
							E.EXPTELEFO,E.EXPDIRECCION,
							P.PERTELEFO,P.PERNOMBRE,P.PERAPELLI,P.PERCORREO,
							E.EXPBANIMG1,
							(SELECT FIRST 1 V.VIDURL
							 FROM EXP_VID V
							 WHERE V.EXPREG=E.EXPREG) AS VIDURL
					FROM EXP_MAEST E
					LEFT OUTER JOIN PER_MAEST P ON P.PERCODIGO=E.PERCODIGO
					LEFT OUTER JOIN EXP_CAT C ON C.CATREG=E.EXPCATEGO
					WHERE E.ESTCODIGO<>3
					ORDER BY C.CATORDEN,E.EXPNOMBRE ";
		...
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			if(strpos(strtolower($expbanimg1),'.gif')===false){
			}else{
				$expbanimg1='';
			}
			...
			$reg['Telefono'] 		= trim($row['EXPTELEFO']);
			$reg['Contacto'] 		= trim($row['EXPDIRECCION']);
			$reg['Banner'] 			= $expbanimg1;
			$reg['Video'] 			= $vidurl;
...
$app->post('/actividades', function()  use ($app){
	...
	try{
		...
		//Cargo las diferentes fechas de la agenda
		$query = "	SELECT DISTINCT A.AGEFCH
					FROM AGE_MAEST A
					WHERE A.ESTCODIGO<>3
					ORDER BY A.AGEFCH ";
		...
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			$reg = array();
			$reg['Fecha'] = BDConvFch($row['AGEFCH']);
			array_push($response["Fechas"],$reg);
		}
		
		//Cargo las diferentes salas de la agenda
		$query = "	SELECT DISTINCT A.AGELUGAR
					FROM AGE_MAEST A
					WHERE A.ESTCODIGO<>3
					ORDER BY A.AGELUGAR ";		
		$Table = sql_query($query,$conn);
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			$reg = array();
			$reg['Nombre'] = trim($row['AGELUGAR']);
			array_push($response["Salas"],$reg);
		}
		...
		if($fltspkreg!=0 && $fltspkreg!=''){
			$fltwhere .=" AND A.SPKREG||',' CONTAINING '$fltspkreg,'";
		}		
		//Filtro de Salas
		if($fltsala!='Todas...' && $fltsala!=''){
			$fltwhere .=" AND AGELUGAR='$fltsala' ";
		}
		//Filtro de Fechas
		if($fltfecha!='Todas...' && $fltfecha!=''){
			$fechaBD = ConvFechaBD($fltfecha);
			$fltwhere .=" AND A.AGEFCH=$fechaBD ";
		}
		
		//Primer Ingreso
		if($fltfecha==''){
			$fechaBD = ConvFechaBD($response["Fechas"][0]['Fecha']);
			$fltwhere .=" AND A.AGEFCH=$fechaBD ";
		}
		
		$query = "	SELECT A.AGEREG,A.AGETITULO, A.AGEDESCRI, A.AGELUGAR, A.AGEFCH, A.AGEHORINI, A.AGEHORFIN, A.ESTCODIGO,
							A.SPKREG, A.AGEPREHAB,A.AGEYOULNK
					FROM AGE_MAEST A
		...
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			$horaini = substr(trim($row['AGEHORINI']),0,5);
			$horafin = substr(trim($row['AGEHORFIN']),0,5);
			if(trim($horafin)=='') $horafin=$horaini;
			
			$titulo = trim($row['AGETITULO']);
			$descri = trim($row['AGEDESCRI']);
			...
			$reg['Titulo'] 			= $titulo;
			$reg['Descripcion']		= $descri;
			$reg['Lugar'] 			= trim($row['AGELUGAR']);
			$reg['Fecha'] 			= $agefch;
			$reg['Hora'] 			= $hora;
			$reg['HoraInicio'] 		= $horaini;
			$reg['HoraFin'] 		= $horafin;
			$reg['Pregunta'] 		= trim($row['AGEPREHAB']);
			$reg['YoutubeLink'] 	= trim($row['AGEYOULNK']);
			$reg['Speakers']		= array();
		...
		$response["ERROR"] 	 	= true;
		$response["MESSAGE"] 	= "Error en actividades ";
		$response["EXCEPTION"] 	=  $e->getMessage();
...
//-----------------------------------------------------------------------------------------------
// CHAT COMPARTIR DATOS
$app->post('/chatcompartir', function()  use ($app){
    $response = array();	
	$json = $app->request->getBody();
	$data = convertJsonObject($json);
	...
	try{
		...
		$query = "	SELECT PERQRITM
					FROM PER_QR  
					WHERE PERCODIGO=$percodigo AND PERQRPER=$percoddst ";		
		$TableCtrl = sql_query($query,$conn);
		if($TableCtrl->Rows_Count>0){
			$existeRel=true;
		}
		...
		$response["ERROR"] 	 	= true;
		$response["MESSAGE"] 	= "Error al compartir los datos de contacto ";
		$response["EXCEPTION"] 	=  $e->getMessage();
...
$app->get('/getexpomapa', function()  use ($app){		
	...
	try{
		...
		$query  = "	SELECT E.EXPREG, E.EXPNOMBRE, E.EXPPOSX, E.EXPPOSY,E.EXPRUBROS,E.EXPAVATAR,
...
$app->post('/reuniones', function()  use ($app){
	...
	try{
		...
		$query = "	SELECT 	PD.PERCODIGO AS PERCODDST, PD.PERNOMBRE AS PERNOMDST, PD.PERAPELLI AS PERAPEDST, PD.PERCOMPAN AS PERCOMDST, PD.PERCORREO AS PERCORDST, PD.PERAVATAR AS PERAVADST,
							...
							R.DESREG,R.REUDESSOL,R.REULINK,
							CASE
								WHEN (R.PERCODSOL=$percodigo AND R.REUESTADO=1) THEN 1 --Enviadas
								WHEN (R.PERCODDST=$percodigo AND R.REUESTADO=1) THEN 2 --Recibida
								WHEN (R.REUESTADO=2) THEN 3 --Agendada
								WHEN (R.REUESTADO=3) THEN 4 --Cancelado
							END AS ESTADOTIPO
					FROM REU_CABE R
		...
		for($i=0; $i<$Table->Rows_Count; $i++){
			...
			$estadotipo = trim($row['ESTADOTIPO']);
			...
			$reg['EstadoTipo'] 			= $estadotipo; //1-Enviada, 2-Recibida, 3-Agendada, 4-Cancelada
			$reg['Fecha'] 				= $fecha;
			$reg['ReunionFecha']		= $reufecha;
			$reg['Hora'] 				= $reuhora;
			$reg['DesafioReg'] 			= $desreg;
			$reg['DesafioSolucion']		= $reudessol;
			$reg['ReunionLink']			= $reulink;
	...
	try{
		...
		if($mesaDisponible==true){
			...
			if($TableDst->Rows_Count>0){
				...
				sendMailDataReunion($reureg,$conn,$trans);
			}
...
$app->post('/perfilguardar', function()  use ($app){
	...
	$perestado 		= (isset($data->PERESTADO))? trim($data->PERESTADO) : '';
	$perciudad 		= (isset($data->PERCIUDAD))? trim($data->PERCIUDAD) : '';
	$perdirecc 		= (isset($data->PERDIRECC))? trim($data->PERDIRECC) : '';
	$perempdes 		= (isset($data->PEREMPDES))? trim($data->PEREMPDES) : '';
		...
		if($opcion==2){ //REGISTRACION
			...
			if($pernombre!='' && $percodigo!='' && $percodigo!='0'){
				$query = "	UPDATE PER_MAEST SET 
							...
							PERCIUDAD='$perciudad', PEREMPDES='$perempdes', PERDIRECC='$perdirecc',
							PERESTADO='$perestado'
							WHERE PERCODIGO=$percodigo ";
				$err = sql_execute($query,$conn);
			}
...
$app->post('/desafios', function()  use ($app){
	...
	try{	
		...
		if($fltarecod!=0){
			$where .= " AND D.PERARECOD=$fltarecod ";
		}
...
$app->post('/qrsendcontactos', function()  use ($app){
	...
	try{
		...
		$mail->AddEmbeddedImage('../imagenes-evento/logo.jpg', 'imglogo'); //cid:imglogo
...
function sendMailDataReunion($reureg,$conn,$trans){
	...
	//http://localhost/webcoordinador/videoconf.php?P=7a728b48fee4e58f87c47e4554747996WTJZd09UVmhNemMyWWpNell6VmpNalV4T0daak9ESTJZV1k0TkdZMFpETXhObUk1TURBNE56RmtZV0kxTmpJMVltUmxNbVpqWldNM05tVmtaVFkxTm1JeGI5MDA4NzFkYWI1NjI1YmRlMmZjZWM3NmVkZTY1NmIx
	...
	$mail->AddAddress(SendMailReporteReuniones, '');
	$mail->Subject = 'Reunion Concretada';
	$mail->AddEmbeddedImage('../imagenes-evento/logo.jpg', 'imglogo'); //cid:imglogo
	...
	$mail->Body ='<!DOCTYPE html>
						<html lang="en" class="loading">
							...
							<div dir="ltr">
								<div style="text-align:center">
									<font color="#2f67a0" style="font-family:arial,sans-serif;font-size:18px;">
										<div style="text-align:center">Reunion concretada el '.$reufecha.' a las '.$reuhora.'</div>
									</font>
								</div>
							</div>
							...
							<div dir="ltr">
								<div style="text-align:center">
									<font color="#2f67a0" style="font-family:arial,sans-serif;font-size:18px;">
										<div style="text-align:center">Perfil 1: '.$pernomsol.' '.$perapesol.' de la empresa '.$percomsol.' (Correo: '.$percorsol.')</div>
									</font>
								</div>
							</div>
							...
							<div dir="ltr">
								<div style="text-align:center">
									<font color="#2f67a0" style="font-family:arial,sans-serif;font-size:18px;">
										<div style="text-align:center">Perfil 2: '.$pernomdst.' '.$perapedst.' de la empresa '.$percomdst.' (Correo: '.$percordst.')</div>
									</font>
								</div>
							</div>							
							<br>
							<div dir="ltr">
								<div style="text-align:center">
									<a href="http://cms.onlife.com.ar/videoconf.php?P='.$parametro.'"  target="_blank">INGRESAR LINK DE VIDEOCONFERENCIA</a>
								</div>
							</body>
						</html>';
	$mail->Send();
