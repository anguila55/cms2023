...
/* -------------------------- ID-SLUG DEL EXPOSITOR GET -------------------------- */
$expreg = (isset($_GET['id']))? $_GET['id'] : 0; //Filtro de Stand
$tmpl->setVariable('expreg'	, $expreg);
/* ------------------------------------ x ----------------------------------- */
$agroreg = (isset($_GET['ag']))? $_GET['ag'] : 0; //Filtro desde Ofertas

if ($agroreg>1){
	$tmpl->setVariable('expreg'	, $agroreg);
	//$tmpl->setVariable('scriptbrwprofile'	, "showPuntosAgroshock($agroreg);"	);
}

if ($tipostand == 0)
{
	if ($expreg != 455){
		$tmpl->setVariable('fondosponsors2'	,'');
		
	}else{
		$tmpl->setVariable('fondosponsors2'	, "background-image: url('../assets-nuevodisenio/img/standfondo2.jpg'); width: 100%; background-size: cover;"	);
