...
if ($percodigo == '') {
	if ($perusuacc == '' || $perpasacc == '') {
		...
		$query = " 	SELECT P.PERCODIGO,P.PERNOMBRE,P.PERAPELLI,P.PERCORREO,P.PERUSUACC,P.PERPASACC,P.ESTCODIGO,P.PERADMIN,P.PERAVATAR,
								P.PERUSADIS,P.PERUSAREU,P.PERUSAMSG,P.PERTIPO,P.PERCLASE,P.PERIDIOMA,P.PERCOMPAN,P.PERCARGO,
								T.TIMREG,T.TIMDESCRI,T.TIMOFFSET,P.PERINGLOG
						FROM PER_MAEST P
...
$query = "	SELECT AGEREG, AGEFCH, AGETITULO, AGEDESCRI, AGEHORINI, AGEHORFIN, AGELUGAR , SPKREG,AGEYOULNK
			FROM AGE_MAEST
			WHERE   ESTCODIGO=1 AND (AGEFCH>'$hoy' OR (AGEFCH='$hoy' AND AGEHORFIN>(CAST('$ahora' AS TIME))))
...
if ($Table->Rows_Count != -1){
	...
	for ($i = 0; $i < $Table->Rows_Count; $i++) {
		...
		$arraycharlas[]=['inicio'=>$charlaini,'fin'=>$charlafin, 'value'=>$agereg];
...
if ($Table->Rows_Count != -1){
	for ($i = 0; $i < $Table->Rows_Count; $i++) {
		...
		$tmpl->setCurrentBlock('actividades1');
...
$percodlog = (isset($_SESSION[GLBAPPPORT.'PERCODIGO']))? trim($_SESSION[GLBAPPPORT.'PERCODIGO']) : '';
	...
	for($i=0; $i<$Table->Rows_Count; $i++){
		...
		if($videoconferencia=='BBB'){
			...
			$arrayreuniones[]=['key'=>$diahor,'value'=>$reureg, 'cant'=>$i+1];
