...
<?
	...
	$secdesing		= VarNullBD($secdesing		, 'N');
	$estcodigo		= VarNullBD($estcodigo		, 'N');
	
	if($seccodigo==0){
		//- - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 		
		//Genero un ID 
		$query 		= 'SELECT GEN_ID(G_ENCGRAL,1) AS ID FROM RDB$DATABASE';
		$TblId		= sql_query($query,$conn,$trans);
		$RowId		= $TblId->Rows[0];			
		$seccodigo 	= trim($RowId['ID']);
		//- - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		
		$query = " 	INSERT INTO ENC_GRAL(ENC_REG,ENC_ID,ENC_TIPPER,ESTCODIGO)
					VALUES($seccodigo,$secdescri,$secdesing,$estcodigo) ";
	}else{
		$query = " 	UPDATE ENC_GRAL SET 
					ENC_REG=$seccodigo, ENC_ID=$secdescri,ENC_TIPPER=$secdesing, ESTCODIGO=$estcodigo
