...
$where1 = ' 1=1 ';
	//Nombre
	if($fltnombre!=''){
		$where1 .= " AND (P.PERNOMBRE CONTAINING '$fltnombre' OR P.PERCORREO CONTAINING '$fltnombre' OR P.PERUSUACC CONTAINING '$fltnombre' OR P.PERAPELLI CONTAINING '$fltnombre' OR P.PERCOMPAN CONTAINING '$fltnombre' OR P.PERCARGO CONTAINING '$fltnombre' OR P.PEREMPDES CONTAINING '$fltnombre' OR P.PERCIUDAD CONTAINING '$fltnombre' OR P.PERESTADO CONTAINING '$fltnombre' OR (EXISTS(SELECT 1 FROM TBL_PAIS TP WHERE TP.PAICODIGO=P.PAICODIGO AND P.PAICODIGO IS NOT NULL AND ( TP.PAIDESCRI CONTAINING '$fltnombre' OR TP.PAIDESCRIING CONTAINING '$fltnombre')))) ";
	}
	...
	if($fltestado!=''){
		if ($fltestado == 0){
			...
			$where1 .= " AND P.ESTCODIGO!=3 ";
			
		}else{

			$where1 .= " AND P.ESTCODIGO=$fltestado ";
	...
	//Tipo de Perfiles
	if($fltpertipo!=''){
		$where1 .= " AND P.PERTIPO=$fltpertipo ";
	}
	//Clase de Perfiles
	if($fltperclase!=''){
		$where1 .= " AND P.PERCLASE=$fltperclase ";
	}
  $arrayperfiles = '';
   if (isset($_POST['percodigo'])){
    foreach ($_POST['percodigo'] as $index => $data) {

      $percodigo = trim($data['id']);
      $arrayperfiles .= $percodigo.',';
    }
    $arrayperfiles .= '0';
    $where1 .= " AND P.PERCODIGO IN ($arrayperfiles)";
  }
...
$queryperfiles = "	SELECT P.PERCOMPAN,P.PERNOMBRE,P.PERAPELLI,P.PERCODIGO,P.TIMOFFSET,P.TIMREG2
				FROM PER_MAEST P
				WHERE ESTCODIGO=1 AND $where1
...
for ($j= 0; $j < $Tableperfiles->Rows_Count; $j++) {
  ...
  $percodlog	= trim($rowperfiles['PERCODIGO']);
	$pernomlog	= utf8_decode(trim($rowperfiles['PERNOMBRE']));
	$perapelog	= utf8_decode(trim($rowperfiles['PERAPELLI']));
	$percompanlog	= utf8_decode(trim($rowperfiles['PERCOMPAN']));
  $timeoffset	= trim($rowperfiles['TIMOFFSET']);
  $timdescri	= trim($rowperfiles['TIMREG2']);
