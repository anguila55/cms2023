...
if ($expreg == 0) {
	...
	$query = " 	INSERT INTO EXP_MAEST(EXPREG,EXPNOMBRE,EXPWEB,EXPMAIL,EXPSTAND,EXPRUBROS,EXPPOSX,EXPPOSY, ESTCODIGO,EXPPOS,EXPLINKED,
	 						EXPFAC,EXPTWI,EXPDIRECCION,EXPTELEFO,EXPYOUTUB,EXPINSTA,EXPPRODDES4,EXPREUTIT,EXPREULNK,)
					VALUES($expreg,$expnombre,$expweb,$expmail,$expstand,$exprubros,$expposx,$expposy,$estcodigo,
						   $exppos,$explinked,$expfac,$exptwi,$expdireccion,$exptelefo,$expyoutub,$expinsta,$expbrolnk,$expreutit,$expreulnk) ";
} else {
	$query = " 	UPDATE EXP_MAEST SET 
					EXPNOMBRE=$expnombre, EXPWEB=$expweb, EXPMAIL=$expmail, EXPSTAND=$expstand, EXPRUBROS=$exprubros,
					EXPPOSX=$expposx,EXPPOSY=$expposy, ESTCODIGO=$estcodigo, EXPPOS=$exppos,
					EXPLINKED = $explinked, EXPFAC = $expfac, EXPTWI  = $exptwi , EXPDIRECCION = $expdireccion , EXPTELEFO = $exptelefo,
					EXPYOUTUB = $expyoutub, EXPINSTA = $expinsta,EXPPRODDES4=$expbrolnk,EXPREUTIT=$expreutit, EXPREULNK=$expreulnk
					WHERE EXPREG=$expreg ";
}
