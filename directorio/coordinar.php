...
if($tiposolicitante == 0){ // solicitante virtual
	...
	$query = "	SELECT P.PERNOMBRE,P.PERAPELLI,P.PERCOMPAN
				FROM PER_MAEST P
				WHERE P.ESTCODIGO=1 AND P.PERCODIGO=$percoddst
	...
	for($i=0; $i<$Table->Rows_Count; $i++){
		...
		$tmpl->setVariable('pernomdst'	, $pernombre	);
		$tmpl->setVariable('perapedst'	, $perapelli	);
		$tmpl->setVariable('percomdst'	, $percompan	);
