...
<?
	...
	require_once 'func/mailchimp.php';
	...
	$query = "	SELECT R.REUREG,R.REUFECHA,R.REUHORA,
						...
						PS.PERCORREO AS SOLCORREO,PS.PERIDIOMA AS SOLIDIOMA,TS.TIMOFFSET AS SOLTIME,
						PD.PERCODIGO AS DESCODIGO,PD.PERNOMBRE AS DESNOMBRE,PD.PERAPELLI AS DESAPELLI,PD.PERCOMPAN AS DESCOMPAN,
						PD.PERCORREO AS DESCORREO,PD.PERIDIOMA AS DESIDIOMA,TD.TIMOFFSET AS DESTIME
				FROM REU_CABE R
				LEFT OUTER JOIN PER_MAEST PS ON PS.PERCODIGO=R.PERCODSOL
				LEFT OUTER JOIN TIM_ZONE TS ON TS.TIMREG=PS.TIMREG
				LEFT OUTER JOIN PER_MAEST PD ON PD.PERCODIGO=R.PERCODDST
				LEFT OUTER JOIN TIM_ZONE TD ON TD.TIMREG=PD.TIMREG
	...
	for($i=0; $i<$Table->Rows_Count; $i++) {
		...
		switch($solidioma){
			case 'ESP':
				$asunto = "Proxima Reunión: $reufecha $reuhoraSol";
			...
			case 'ING':
				...
				break;	
			case 'POR':
				$asunto = "Próxima reunião: $reufecha $reuhoraSol";
		...
		switch($desidioma){
			case 'ESP':
				$asunto = "Proxima Reunión: $reufecha $reuhoraDes";
			...
			case 'ING':
				...
				break;
			case 'POR':
				$asunto = "Próxima reunião: $reufecha $reuhoraDes";
