...
//-------------------------------------------------------------------------------------------------------
function init_login($perusuacc, $perpasacc)
{
    $time = time() + (30 * 24 * 60 * 60);
    session_set_cookie_params($time);
    setcookie(session_name(), session_id(), $time);
    setcookie('perusuacc', $perusuacc, $time);
    setcookie('perpasacc', $perpasacc, $time);
...
function validateLogin($percodigo) {
    if ($percodigo != "" && isset($_COOKIE[GLBAPPPORT . 'PERCODIGO'], $_COOKIE['perusuacc'], $_COOKIE['perpasacc'])) {
        $percodigo = $_COOKIE[GLBAPPPORT . 'PERCODIGO'];
        $perusuacc = $_COOKIE['perusuacc'];
        $perpasacc = $_COOKIE['perpasacc'];
    }
}